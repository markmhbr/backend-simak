...
export class MandalaService implements OnModuleInit {
  ...
  async requestReset2FAPegawai(identifier: string, pass: string) {
    const pegawai = await this.prisma.pegawai.findFirst({
      where: {
        OR: [
          { nip: identifier },
    ...
    if (!email || email.trim() === '') {
      throw new BadRequestException('Email tidak terdaftar pada akun Anda. Silakan hubungi admin Anda.');
    }
  ...
  async verifyReset2FAPegawai(resetToken: string, code: string) {
    try {
  ...
  async getRombonganBelajarForMandala(
    sekolahId: string | undefined,
    query: {
      type?: string;
      limit: number;
  ...
  async getRombelAnggotaForMandala(rombelId: string) {
  ...
  async getPesertaDidikPresenceForMandala(sekolahId: string, date: Date) {
    ...
    // Cari semester_id terbaru dari rombongan belajar sekolah terkait
    const latestRombel = await this.prisma.rombonganBelajar.findFirst({
      where: sekolahId ? { sekolah_id: sekolahId } : {},
      select: { semester_id: true },
      orderBy: { semester_id: 'desc' },
  ...
  async getSemestersForMandala() {
    // Cari semester_id terbaru dari rombongan_belajar
    const latestRombel = await this.prisma.rombonganBelajar.findFirst({
      select: { semester_id: true },
      orderBy: { semester_id: 'desc' },
    });
    ...
    // Fallback jika tidak ada data di rombongan_belajar
