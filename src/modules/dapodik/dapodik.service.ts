...
export class DapodikService {
  ...
  async uploadLogo(sekolahId: string, file: Express.Multer.File) {
    ...
    // Hapus logo lama untuk menghemat penyimpanan
    if (fs.existsSync(destDir)) {
      try {
        const files = fs.readdirSync(destDir);
  ...
  async getUpdateGtk(sekolahId: string, since?: string) {
    const where: any = {
      sekolah_id: sekolahId,
  ...
  async getUpdatePesertaDidik(sekolahId: string, since?: string) {
    const where: any = {
      sekolah_id: sekolahId,
      status: 'Aktif',
      jenis_keluar_id: null,
  ...
  async registerGtkFace(sekolahId: string, id: string, embedding: number[]) {
  ...
  async registerStudentFace(sekolahId: string, id: string, embedding: number[]) {
    const pengguna = await this.prisma.pengguna.findFirst({
      where: { peserta_didik_id: id },
    });
  ...
  async getGtkFace(sekolahId: string, id: string) {
    const pengguna = await this.prisma.pengguna.findFirst({
      where: { ptk_id: id },
  ...
  async getStudentFace(sekolahId: string, id: string) {
    const pengguna = await this.prisma.pengguna.findFirst({
      where: { peserta_didik_id: id },
      select: { face_embedding: true },
    });
  ...
  async identifyFace(sekolahId: string, embedding: number[]) {
    const users = await this.prisma.pengguna.findMany({
      where: {
        sekolah_id: sekolahId,
