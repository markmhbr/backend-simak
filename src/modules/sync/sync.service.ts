...
export class SyncService {
  ...
  async syncBidangStudi(sekolahId: string, dataRows: any[]) {
    let successCount = 0;
    for (const b of dataRows) {
      if (b.bidang_studi_id === null || b.bidang_studi_id === undefined) continue;
  ...
  async syncLembSertifikasi(sekolahId: string, dataRows: any[]) {
    let successCount = 0;
    for (const l of dataRows) {
      if (!l.kode_lemb_sert) continue;
