...
export class PerpustakaanService {
  ...
  // =========================================================================
  // 0. SUMMARY / STATISTIK PERPUSTAKAAN
  // =========================================================================

  async getDashboardStats(sekolahId: string) {
    ...
    const [
      ...
      this.prisma.peminjaman.count({
        where: {
          ...
          status: { in: [1, 2, 4] }, // Dipinjam, Sebagian Dikembalikan, Terlambat
        },
      }),
      this.prisma.peminjaman.count({
        where: {
  ...
  async getKategoriList(sekolahId: string) {
  ...
  async getKategoriById(sekolahId: string, id: string) {
    ...
    if (!data) {
      throw new NotFoundException('Kategori buku tidak ditemukan.');
    }
    return data;
  }

  async createKategori(sekolahId: string, dto: CreateKategoriBukuDto) {
    ...
    if (existing) {
      throw new ConflictException(`Kategori '${dto.nama}' sudah ada di sekolah ini.`);
    }
  ...
  async updateKategori(sekolahId: string, id: string, dto: UpdateKategoriBukuDto) {
    ...
    if (dto.nama) {
      ...
      if (duplicate) {
        throw new ConflictException(`Kategori '${dto.nama}' sudah ada.`);
      }
    }
  ...
  async deleteKategori(sekolahId: string, id: string) {
    await this.getKategoriById(sekolahId, id);
    ...
    if (countBuku > 0) {
      throw new BadRequestException(
        `Kategori tidak dapat dihapus karena masih digunakan oleh ${countBuku} buku.`,
      );
    }
  ...
  async getBukuById(sekolahId: string, id: string) {
  ...
  async createBuku(sekolahId: string, dto: CreateBukuDto) {
    ...
    if (!kategori) {
      throw new BadRequestException('Kategori buku tidak valid.');
    }
    ...
    if (existingKode) {
      throw new ConflictException(`Kode buku '${dto.kode}' sudah digunakan di sekolah ini.`);
    }
    ...
    if (tersedia > jumlah) {
      throw new BadRequestException('Stok tersedia tidak boleh melebihi total jumlah buku.');
    }
  ...
  async updateBuku(sekolahId: string, id: string, dto: UpdateBukuDto) {
    ...
    if (dto.kode && dto.kode.trim() !== existing.kode) {
      ...
      if (duplicateKode) {
        throw new ConflictException(`Kode buku '${dto.kode}' sudah digunakan.`);
      }
    }
  ...
  async deleteBuku(sekolahId: string, id: string) {
    ...
    if (activeLoan) {
      throw new BadRequestException('Buku sedang dalam proses peminjaman aktif dan tidak dapat dihapus.');
    }
  ...
  // =========================================================================
  // 3. PEMINJAMAN & PENGEMBALIAN BUKU (TRANSACTIONAL CRUD)
  // =========================================================================
  ...
  async getPeminjamanById(sekolahId: string, id: string) {
  ...
  async createPeminjaman(sekolahId: string, dto: CreatePeminjamanDto) {
    const hasPd = Boolean(dto.peserta_didik_id);
    const hasPtk = Boolean(dto.ptk_id);

    if ((hasPd && hasPtk) || (!hasPd && !hasPtk)) {
      throw new BadRequestException(
        'Peminjam harus ditentukan tepat salah satu: peserta_didik_id atau ptk_id.',
      );
    }
    ...
    if (tglJatuhTempo < tglPinjam) {
      throw new BadRequestException('Tanggal jatuh tempo tidak boleh sebelum tanggal pinjam.');
    }
    ...
    if (dto.peserta_didik_id) {
      ...
      if (!pd) {
        throw new BadRequestException('Peserta didik tidak ditemukan di sekolah ini.');
      }
    } else if (dto.ptk_id) {
      ...
      if (!ptk) {
        throw new BadRequestException('PTK / Guru tidak ditemukan di sekolah ini.');
      }
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!nomorPeminjaman) {
        ...
        if (duplicateNo) {
          throw new ConflictException(`Nomor peminjaman '${nomorPeminjaman}' sudah digunakan.`);
        }
      }
      ...
      for (const item of dto.items) {
        ...
        if (!buku) {
          throw new NotFoundException(`Buku dengan ID ${item.buku_id} tidak ditemukan.`);
        }
        if (buku.status !== 1) {
          throw new BadRequestException(`Buku '${buku.judul}' sedang tidak aktif.`);
        }
        if (buku.tersedia < item.jumlah) {
          throw new BadRequestException(
            `Stok buku '${buku.judul}' tidak mencukupi. Tersedia: ${buku.tersedia}, Diminta: ${item.jumlah}.`,
          );
        }
  ...
  async prosesPengembalian(sekolahId: string, peminjamanId: string, dto: PengembalianDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (peminjaman.status === 3) {
        throw new BadRequestException('Peminjaman ini sudah selesai dikembalikan.');
      }
      if (peminjaman.status === 5) {
        throw new BadRequestException('Peminjaman ini berstatus dibatalkan.');
      }

      const tglKembali = dto.tanggal_kembali ? new Date(dto.tanggal_kembali) : new Date();

      for (const returnItem of dto.items) {
        const detail = peminjaman.detail_peminjaman.find(
          (d) => d.detail_peminjaman_id === returnItem.detail_peminjaman_id,
        );

        if (!detail) {
          throw new NotFoundException(
            `Detail peminjaman ID ${returnItem.detail_peminjaman_id} tidak valid untuk transaksi ini.`,
          );
        }
        ...
        if (returnItem.jumlah_kembali > sisaDipinjam) {
          throw new BadRequestException(
            `Jumlah pengembalian (${returnItem.jumlah_kembali}) melebihi sisa pinjaman (${sisaDipinjam}).`,
          );
        }
      ...
      if (allReturned) {
        ...
        newStatus = 2; // Sebagian Dikembalikan
      }
  ...
  async batalkanPeminjaman(sekolahId: string, id: string) {
    return this.prisma.$transaction(async (tx) => {
      const peminjaman = await tx.peminjaman.findFirst({
      ...
      if (peminjaman.status === 5) {
        throw new BadRequestException('Peminjaman sudah dibatalkan sebelumnya.');
      }
      if (peminjaman.status === 3) {
        throw new BadRequestException('Peminjaman sudah selesai, tidak dapat dibatalkan.');
      }

      // Kembalikan sisa buku yang belum dikembalikan ke stok tersedia
      ...
      return tx.peminjaman.update({
        ...
        data: { status: 5 }, // 5: Dibatalkan
      });
    });
  }

  async deletePeminjaman(sekolahId: string, id: string) {
    return this.prisma.$transaction(async (tx) => {
      const peminjaman = await tx.peminjaman.findFirst({
        where: { peminjaman_id: id, sekolah_id: sekolahId },
        include: { detail_peminjaman: true },
      ...
      // Jika status masih aktif/dipinjam, kembalikan stok sebelum dihapus
  ...
  async getKunjunganById(sekolahId: string, id: string) {
    ...
    if (!data) {
      throw new NotFoundException('Data kunjungan perpustakaan tidak ditemukan.');
    }
    return data;
  }

  async checkInKunjungan(sekolahId: string, dto: CreateKunjunganDto) {
    const hasPd = Boolean(dto.peserta_didik_id);
    const hasPtk = Boolean(dto.ptk_id);

    if ((hasPd && hasPtk) || (!hasPd && !hasPtk)) {
      throw new BadRequestException(
        'Pengunjung harus ditentukan tepat salah satu: peserta_didik_id atau ptk_id.',
      );
    }
  ...
  async smartScanKunjungan(sekolahId: string, dto: CreateKunjunganDto) {
    const hasPd = Boolean(dto.peserta_didik_id);
    const hasPtk = Boolean(dto.ptk_id);
    ...
    // Cari apakah pengunjung memiliki kunjungan aktif hari ini yang belum check-out (jam_keluar === null)
    ...
    if (activeVisit) {
      // 🚪 OTOMATIS CHECK-OUT (Keluar Perpustakaan)
      ...
      // 🚀 OTOMATIS CHECK-IN (Masuk Perpustakaan Baru)
  ...
  async checkOutKunjungan(sekolahId: string, id: string, dto: CheckOutKunjunganDto) {
    const existing = await this.getKunjunganById(sekolahId, id);
    if (existing.jam_keluar) {
      throw new BadRequestException('Kunjungan ini sudah melakukan check-out sebelumnya.');
    }
  ...
  async updateKunjungan(
    sekolahId: string,
    id: string,
    data: { keperluan?: string; keterangan?: string; jam_masuk?: string; jam_keluar?: string },
  ) {
  ...
  async deleteKunjungan(sekolahId: string, id: string) {
    await this.getKunjunganById(sekolahId, id);
  ...
  async getLiterasiById(sekolahId: string, id: string) {
    ...
    if (!data) {
      throw new NotFoundException('Data literasi perpustakaan tidak ditemukan.');
    }
    return data;
  }

  async createLiterasi(sekolahId: string, dto: CreateLiterasiDto) {
    if (dto.halaman_sampai < dto.halaman_dari) {
      throw new BadRequestException('halaman_sampai tidak boleh lebih kecil dari halaman_dari.');
    }
  ...
  async updateLiterasi(sekolahId: string, id: string, dto: UpdateLiterasiDto) {
  ...
  async deleteLiterasi(sekolahId: string, id: string) {
