...
async function bootstrap() {
  ...
  const handleMenu = async (choice: string) => {
    try {
      switch (choice) {
        case '1':
...
async function listKeys(
  ...
  const keys: any[] = await service.getAllKeys(searchKeyword);
...
async function createNewKey(service: AppKeyService, question: (q: string) => Promise<string>) {
  console.log('\n➕ MEMBUAT KEY BARU');
  ...
  const result: any = await service.createKey(namaApp.trim(), sekolahId.trim());
  console.log('\n✅ BERHASIL DIBUAT!');
...
async function regenerateKey(service: AppKeyService, question: (q: string) => Promise<string>) {
