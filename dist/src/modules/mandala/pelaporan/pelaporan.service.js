...
let PelaporanService = class PelaporanService {
            ...
            while (currentKode && maxDepth > 0) {
                ...
                switch (wil.id_level_wilayah) {
                    ...
                    case 3:
                        ...
                        break;
                    case 2:
                        result.kabupaten_kota = wil.nama;
                        break;
                    case 1:
            ...
            if (excelDoc) {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    if (expectedHeaders && expectedHeaders.length > 0) {
                        ...
                        tableHtml = (0, excel_html_generator_helper_1.generateDynamicHtmlTable)(expectedHeaders, rows, pelaporan.judul);
    ...
    async exportAllSekolahExcel(cadisdikId, pelaporanId) {
        ...
        const formattedData = allRows.map((r, index) => {
            const rowObj = { 'No': String(index + 1) };
            expectedHeaders.forEach(h => {
    ...
    async updatePelaporan(cadisdikId, pelaporanId, dto) {
        ...
        if (uploadedDocsCount > 0) {
            throw new common_1.BadRequestException('Tidak dapat mengubah data pelaporan karena sudah ada sekolah yang mengunggah dokumen.');
        }
        return await this.prisma.$transaction(async (tx) => {
