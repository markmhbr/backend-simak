...
let ReferenceService = class ReferenceService {
    ...
    async getMstWilayah(search, limit = 50) {
    ...
    async getWilayahByParent(level, parentCode) {
        let mst_kode_wilayah = parentCode ? parentCode.trim().padEnd(8, ' ') : undefined;
        if (level === 1) {
    ...
    async getAllOptions() {
        const [agama, bank, jabatan_ptk, jenis_ptk, keahlian_laboratorium, lembaga_pengangkat, pangkat_golongan, status_kepegawaian, sumber_gaji, alat_transportasi, jenis_cita, jenis_hobby, alasan_layak_pip, jenis_pendaftaran, jenis_tinggal, jenis_keluar, kebutuhan_khusus, pekerjaan, jenjang_pendidikan, penghasilan,] = await Promise.all([
            this.getAgama(),
            this.getBank(),
            this.getJabatanPtk(),
            this.getJenisPtk(),
    ...
    async resolveGtk(gtk) {
        if (!gtk)
            return null;
        const [agama, bank, jabatan_ptk, jenis_ptk, keahlian_lab, wilayah, lembaga_pengangkat, pangkat_golongan, status_kepegawaian, sumber_gaji, pekerjaan_suami_istri, kebutuhan_khusus, jenis_keluar,] = await Promise.all([
        ...
        if (gtk.rwy_sertifikasi && gtk.rwy_sertifikasi.length > 0) {
            resolvedSertifikasi = await Promise.all(gtk.rwy_sertifikasi.map(async (s) => {
        ...
        if (gtk.rwy_kepangkatan && gtk.rwy_kepangkatan.length > 0) {
            resolvedKepangkatan = await Promise.all(gtk.rwy_kepangkatan.map(async (k) => {
    ...
    async resolvePesertaDidik(pd) {
        if (!pd)
            return null;
        const [agama, bank, wilayah, alat_transportasi, jenis_cita, jenis_hobby, alasan_layak_pip, jenis_pendaftaran, jenis_tinggal, jenis_keluar, rombongan_belajar, kebutuhan_khusus, kebutuhan_khusus_ayah, kebutuhan_khusus_ibu, pekerjaan, pekerjaan_ayah, pekerjaan_ibu, pekerjaan_wali, jenjang_pendidikan_ayah, jenjang_pendidikan_ibu, jenjang_pendidikan_wali, penghasilan_ayah, penghasilan_ibu, penghasilan_wali,] = await Promise.all([
