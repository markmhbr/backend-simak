...
let SyncService = SyncService_1 = class SyncService {
    ...
    async syncPesertaDidik(sekolahId, dataRows) {
        ...
        for (const p of dataRows) {
            ...
            const payload = {
                ...
                status_hidup_ayah: (Number(p.status_hidup_ayah) === 1 ||
                    ...
                    (typeof p.nama_ayah === 'string' && (p.nama_ayah.toLowerCase().includes('alm') || p.nama_ayah.toLowerCase().includes('meninggal') || p.nama_ayah.toLowerCase().includes('wafat') || p.nama_ayah.toLowerCase().includes('almarhum')))) ? 1 : 0,
                status_hidup_ibu: (Number(p.status_hidup_ibu) === 1 ||
                    ...
                    (typeof p.nama_ibu_kandung === 'string' && (p.nama_ibu_kandung.toLowerCase().includes('alm') || p.nama_ibu_kandung.toLowerCase().includes('meninggal') || p.nama_ibu_kandung.toLowerCase().includes('wafat') || p.nama_ibu_kandung.toLowerCase().includes('almarhum')))) ? 1 : 0,
                soft_delete: this.parseNumber(p.soft_delete),
                    ...
                    for (const a of anakList) {
                        ...
                        if (existing) {
                            await this.prisma.anak.update({
                                where: { anak_id: a.anak_id },
                                data: {
                                    status_anak_id: this.parseNumber(a.status_anak_id) ?? 1,
                                    jenjang_pendidikan_id: this.parseNumber(a.jenjang_pendidikan_id) ?? 6,
                                    nik: a.nik || null,
                                    nisn: a.nisn || null,
                                    nama: a.nama || 'Tanpa Nama',
                                    jenis_kelamin: a.jenis_kelamin || 'L',
                                    tempat_lahir: a.tempat_lahir || null,
                                    tanggal_lahir: this.parseDate(a.tanggal_lahir) || new Date(),
                                    tahun_masuk: this.parseNumber(a.tahun_masuk),
                                    create_date: this.parseDate(a.create_date) || new Date(),
                                    last_update: this.parseDate(a.last_update) || new Date(),
                                    soft_delete: this.parseNumber(a.soft_delete) ?? 0,
                                    last_sync: this.parseDate(a.last_sync) || new Date(),
                                    updater_id: a.updater_id || g.ptk_id,
                                },
                            });
                        }
                        else {
    ...
    async syncRwyKepangkatan(sekolahId, dataRows) {
        let successCount = 0;
        for (const r of dataRows) {
            if (!r.riwayat_kepangkatan_id)
                continue;
            const payload = {
                sekolah_id: sekolahId,
                ptk_id: r.ptk_id || null,
